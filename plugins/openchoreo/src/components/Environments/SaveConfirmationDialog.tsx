    ...
    <Dialog open={open} onClose={onCancel} maxWidth="md" fullWidth>
      ...
      <DialogContent dividers>
        ...
        >
          ...
          {/* Workload Overrides */}
          {changes.workload && changes.workload.length > 0 && (
            <Box mb={2}>
              <Typography
                variant="subtitle2"
                style={{ fontWeight: 'bold', marginBottom: 8 }}
              >
                Workload Overrides ({changes.workload.length}{' '}
                {changes.workload.length === 1 ? 'change' : 'changes'}):
              </Typography>
              {renderChangesList(changes.workload)}
            </Box>
          )}
