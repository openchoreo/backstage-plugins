...
export const openchoreoIncrementalConfigSchema = z.object({
  ...
  /**
   * Number of entities to process in each batch. Must be between 1 and 100.
   * The OpenChoreo API caps page size at 100 (LimitParam maximum), so values
  ...
  chunkSize: z
    ...
    .max(100)
    .default(100)
    .describe('Number of entities per batch (max 100, API page size cap)'),
