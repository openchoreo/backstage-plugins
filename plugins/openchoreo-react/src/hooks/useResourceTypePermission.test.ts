...
import { useResourceTypePermission } from './useResourceTypePermission';
...
describe('useResourceTypePermission', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('returns canCreate=true when allowed', () => {
    ...
    const { result } = renderHook(() => useResourceTypePermission());
  ...
  it('does not pass resourceRef to usePermission', () => {
    mockUsePermission.mockReturnValue({ allowed: true, loading: false });
    renderHook(() => useResourceTypePermission());
