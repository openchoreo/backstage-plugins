...
import { useComponentCreateContextPermission } from './useComponentCreateContextPermission';
...
describe('useComponentCreateContextPermission', () => {
  ...
  it.each([
    ...
    { namespace: 'acme', project: 'payments', componentType: undefined },
  ])(
  ...
  it('forwards explicit componentType.kind to the backend', async () => {
    mockUsePermission.mockReturnValue({ allowed: true, loading: false });
    mockFetch.mockResolvedValue({
      ok: true,
      json: async () => ({ allowed: true }),
