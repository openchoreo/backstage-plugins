...
import { useHookCreatePermission } from './useHookCreatePermission';
...
describe('useHookCreatePermission', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('returns canCreate=true when allowed', () => {
    ...
    const { result } = renderHook(() => useHookCreatePermission());
  ...
  it('returns denied tooltip when not allowed', () => {
    ...
    expect(result.current.createDeniedTooltip).toContain('hook');
  });

  it('does not pass resourceRef to usePermission', () => {
    mockUsePermission.mockReturnValue({ allowed: true, loading: false });
    renderHook(() => useHookCreatePermission());
