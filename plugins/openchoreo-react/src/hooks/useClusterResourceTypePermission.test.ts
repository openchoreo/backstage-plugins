...
import { useClusterResourceTypePermission } from './useClusterResourceTypePermission';
...
describe('useClusterResourceTypePermission', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('returns canCreate=true when allowed', () => {
    ...
    const { result } = renderHook(() => useClusterResourceTypePermission());
  ...
  it('does not pass resourceRef to usePermission', () => {
    mockUsePermission.mockReturnValue({ allowed: true, loading: false });
    renderHook(() => useClusterResourceTypePermission());
