...
import { useNotificationChannelPermission } from './useNotificationChannelPermission';
...
describe('useNotificationChannelPermission (org-level, no resource)', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('returns canCreate=true when allowed', () => {
    ...
    const { result } = renderHook(() => useNotificationChannelPermission());
  ...
  it('does not pass resourceRef to usePermission', () => {
    mockUsePermission.mockReturnValue({ allowed: true, loading: false });
    renderHook(() => useNotificationChannelPermission());
