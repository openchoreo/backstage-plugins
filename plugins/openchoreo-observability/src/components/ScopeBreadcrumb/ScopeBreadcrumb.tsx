...
/**
 * A namespace → project → component selection, shared by the standalone
 * Insights pages. Each level is optional; an absent deeper level means
 * "all" (aggregated) at the level above.
 */
...
export interface ScopeBreadcrumbProps {
  ...
  /**
   * Prefix for the catalog query cache keys. Pages that show the same
...
/**
 * Scope picker rendered as the `subtitle` of a standalone Insights page header:
 * one pill per selected level, where clicking the name navigates to that level
 * (dropping deeper selections) and the caret switches to a sibling.
 */
export const ScopeBreadcrumb: FC<ScopeBreadcrumbProps> = ({
  scope,
  onScopeChange,
  ...
  // Options carry the raw entity name (used for navigation + API calls) and the
  // catalog `metadata.title` as the display label, so the breadcrumb shows
  ...
  const { data: namespaces = [], loading: nsLoading } = useOpenChoreoQuery<
    Option[]
  >([`${queryKeyPrefix}-namespaces`], async () => {
