...
export class ObservabilityService {
  ...
  /**
   * Resolves the observer URL for a project by fetching a component and getting its observer URL.
   * TODO: This is a temporary workaround until the controlplane endpoint is updated
   *
   * @param orgName - The organization name
   * @param projectName - The project name
   * @param environmentName - The name of the environment
   * @param context - A context string for logging (e.g., "project X", "alert Y")
   * @returns Promise<string> - The resolved observer URL
   * @throws Error if environment is not specified, no components found, or observer URL cannot be resolved
   */
  private async resolveObserverUrlForProject(
    orgName: string,
    projectName: string,
    ...
    if (!environmentName) {
      throw new Error(
        'Environment and components within the project are required to fetch observability data',
      );
    }
    ...
    this.logger.debug(
      `Using component ${firstComponent.name} to get observer URL for ${context}`,
    );
    ...
    this.logger.debug(`Resolved observer URL: ${observerUrl} for ${context}`);
   ...
   * @param options - Optional parameters for filtering reports
   * @param options.limit - The maximum number of reports to return
   * @param options.startTime - The start time for report query
   * @param options.endTime - The end time for report query
   * @param options.status - Filter by report status (pending/completed/failed)
  ...
  async fetchRCAReportsByProject(
    ...
    try {
      this.logger.debug(
        `Fetching RCA reports for project ${projectName} in environment ${
          environmentName || 'all'
        }`,
      );

      const observerUrl = await this.resolveObserverUrlForProject(
        orgName,
        projectName,
        environmentName,
      ...
      this.logger.debug(
        `Sending RCA reports request to ${observerUrl}/api/rca-reports/project/${projectId} with limit: ${
          options?.limit || 100
        }`,
      );
   ...
   * @param projectId - The ID of the project (for getting observer URL)
   * @param environmentId - The ID of the environment (for getting observer URL)
   * @param orgName - The organization name
   * @param projectName - The project name
   * @param environmentName - The name of the environment
   * @param options - Optional parameters
   * @param options.version - Specific version number of the report to retrieve
   * @returns Promise with detailed RCA report data
   */
  async fetchRCAReportByAlert(
    ...
    try {
      this.logger.debug(
        `Fetching RCA report for alert ${alertId}${
