...
import type { ClusterTraitResponse } from '@openchoreo/backstage-plugin-common';
import {
  getName,
  getDisplayName,
  getDescription,
...
type ClusterTraitListResponse = ApiListResponse<ClusterTraitResponse>;
    ...
    try {
      ...
      const allTraits = await fetchAllPages(cursor =>
        client
          ...
          .then(res => {
            assertApiResponse(res, 'fetch cluster traits');
            return res.data;
          }),
      );
