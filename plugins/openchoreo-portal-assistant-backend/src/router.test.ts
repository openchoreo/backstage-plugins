...
/**
 * Spin up a minimal Node http.Server as a mock upstream "perch-agent".
...
describe('openchoreo-portal-assistant-backend router', () => {
  ...
  describe('forwardStream edge cases', () => {
    ...
    it('aborts the upstream fetch when the client disconnects before the stream ends', async () => {
      // The router wires res.on('close') -> clientAbort.abort(). Drive a
      // raw HTTP request and call ``req.destroy()`` after the first
      // chunk to trigger that path. This is the ``onResClose`` body
      // plus the ``clientAbort.signal.aborted`` branch in the pipe-await
      // catch.
      ...
      upstream.setHandler((req, res) => {
        ...
        res.writeHead(200, { 'content-type': 'application/x-ndjson' });
        res.write('{"type":"message_chunk","content":"hi"}\n');
        // Hold the stream open until the client disconnect propagates.
      });
      ...
      try {
        ...
        expect(logger.info).toHaveBeenCalledWith(
          expect.stringMatching(/client closed before stream end rid=/),
        );
    ...
    it('forwards the upstream status code on streaming responses', async () => {
      // Confirm the streaming path passes through non-200 codes. The
      // streaming route can legitimately return a 4xx with an ndjson
      // error body; we must not rewrite the status.
      upstream.setHandler((_req, res) => {
