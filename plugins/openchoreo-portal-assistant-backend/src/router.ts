...
/**
 * Module-level keep-alive HTTP/1.1 dispatcher reused across every
 * forwarded chat / warmup call. Without this, Node's global ``fetch``
 * (also undici under the hood) opens a fresh TCP + TLS connection per
 * request and pays the ~50-300 ms handshake every time a user submits
 * a turn.
 *
 * Tuned for in-cluster traffic, not the public internet:
 *   - ``keepAliveTimeout``: how long to keep an idle socket warm.
 *     60 s comfortably outlives the gap between a /warmup and the
 *     follow-up /chat without holding sockets open through long
 *     periods of inactivity.
 *   - ``keepAliveMaxTimeout``: hard cap so a single socket can't be
 *     reused indefinitely (avoids accumulated TCP-level issues).
 *   - ``connections``: per-origin parallelism. Backstage backends do
 *     not usually run dozens of perch chats concurrently; 32 is
 *     enough to absorb a small burst without sustaining a large pool.
 *   - ``bodyTimeout: 0``: disable undici's per-body inactivity
 *     timeout. Our /chat ndjson stream is silent during long LLM
 *     thinking pauses (no chunks for 10-20 s while a tool runs is
 *     normal); without this the dispatcher would tear the socket
 *     down mid-turn. The orchestrator-level AbortSignal.any composed
 *     in ``forwardStream`` is the authoritative deadline.
 */
...
export interface RouterOptions {
  ...
  targetUrl: string;
  /**
   * Hard timeout (ms) for non-streaming forwards (`/warmup`).
...
/**
 * Non-streaming forwarder for ``/warmup``. Reads the full upstream body
 * and relays it as JSON. Same body-shape as a passthrough — no
 * transformation.
 */
async function forwardJson(
  ...
  // Read the body BEFORE writing headers/status. If the upstream resets
  // the connection mid-body, ``upstream.text()`` rejects — and once
  // we've already called ``res.status()`` + ``applyResponseHeaders()``
  // we can no longer return a clean 502 envelope (Express has flushed
  // the head). Reading first lets us convert a body-read failure into
  // the same BAD_GATEWAY shape we use for the initial fetch failure.
  let text: string;
  try {
    ...
    const timedOut = isTimeoutError(err);
    logger.error(
      `perch-backend: upstream body read ${
        timedOut ? 'timed out' : 'failed'
      } rid=${requestId} path=${upstreamPath}: ${(err as Error).message}`,
    );
    if (timedOut) {
      res.status(504).json({
        error: 'GATEWAY_TIMEOUT',
