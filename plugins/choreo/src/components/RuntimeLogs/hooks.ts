...
import { getRuntimeLogs, getEnvironments, calculateTimeRange } from '../../api/runtime-logs';
import { 
  LogEntry, 
  Environment, 
  RuntimeLogsFilters, 
...
export function useRuntimeLogs(filters: RuntimeLogsFilters, pagination: RuntimeLogsPagination) {
  const { entity } = useEntity();
  const discovery = useApi(discoveryApiRef);
  const identity = useApi(identityApiRef);
  ...
  const fetchLogs = useCallback(async (reset: boolean = false) => {
    if (loading || !filters.environmentId) return;
    ...
    try {
      ...
      const response = await getRuntimeLogs(entity, discovery, identity, {
        namespace: filters.environmentId, // Using environmentId as namespace for now
...
export function useFilters() {
  ...
  const updateFilters = useCallback((newFilters: Partial<RuntimeLogsFilters>) => {
