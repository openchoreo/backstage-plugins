...
`perchAgentApiRef` API factory through Backstage's idiomatic apiRef
pattern, so consumers do `useApi(perchAgentApiRef)` instead of
...
- Mutating tools — the agent is read-only. Use the OpenChoreo UI or
  CLI for writes.
