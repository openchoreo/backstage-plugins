...
import { EntityKindFilter, useEntityList, catalogApiRef } from '@backstage/plugin-catalog-react';
...
function useEntityKindFilter(opts: { initialFilter: string }): {
  ...
  const selectedKindLabel = kindDisplayNames[selectedKind.toLowerCase()] || allKinds.get(selectedKind) || selectedKind;
...
export const ChoreoEntityKindPicker = (props: ChoreoEntityKindPickerProps) => {
  ...
  const filteredKinds = allowedKinds
    ? new Map([...customKindsMap].filter(([key]) =>
        allowedKinds.some(allowed => allowed.toLowerCase() === key.toLowerCase())
      ))
    : customKindsMap;
