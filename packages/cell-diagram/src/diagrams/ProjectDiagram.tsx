...
export function ProjectDiagram(props: ProjectDiagramProps) {
  ...
  const observationSummary = useRef<ObservationSummary | null>(null);
  ...
  const drawDiagram = () => {
    ...
    const cellNode = new CellModel(
      ...
      Array.from(
        (
          diagramData.nodes.connectorNodes as Map<string, ConnectionModel>
        ).values(),
      ),
      Array.from(
        (
          diagramData.nodes.connectionNodes as Map<string, ConnectionModel>
        ).values(),
      ),
    );
    ...
    setTimeout(() => {
      ...
      if (diagramEngine.getCanvas()?.getBoundingClientRect !== undefined) {
  ...
  const showDiagramLayers =
    (showControls &&
      (observationSummary.current?.requestCount.max ?? 0) > 0 &&
      !previewMode) ||
    false;
  ...
  const ctx = {
    ...
    componentMenu,
    zoomLevel,
    observationSummary: observationSummary.current ?? undefined,
    defaultDiagramLayer,
  ...
  let diagramContent;
  if (diagramEngine?.getModel() && diagramModel) {
    diagramContent = (
      <>
        <CanvasWidget
        ...
        {showControls && !previewMode && (
          <DiagramControls engine={diagramEngine} animation={animation} />
        )}
        {showDiagramLayers && (
          <DiagramLayers
            animation={animation}
            tooltips={customTooltips?.diagramLayers}
          />
        )}
    ...
    diagramContent = <PromptScreen userMessage={userMessage} />;
  } else {
    diagramContent = <CircularProgress sx={{ color: Colors.PRIMARY }} />;
  }
