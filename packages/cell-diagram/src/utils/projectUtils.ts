...
import {
  ...
  getCellPortMetadata,
  getCellLinkName,
} from '../components/Cell/cell-util';
import {
  BORDER_GAP,
  BORDER_NODE,
  CIRCLE_WIDTH,
...
import {
  getExternalLinkName,
  getExternalNodeName,
} from '../components/External/external-node-util';
import { DagreEngine } from '../resources/Dagre/DagreEngine';

// Project diagram engine utils
export function getDiagramDataFromProject(
  project: Project,
): ProjectDiagramData {
  const componentNodes: Map<string, CommonModel> =
    generateComponentNodes(project);
  const connectorNodes: Map<string, ConnectionModel> =
    generateConnectorNodes(project);
  const connectionNodes: Map<string, ConnectionModel> =
    generateConnectionNodes(project);
  ...
  const componentLinks: Map<string, ComponentLinkModel> =
    generateComponentLinks(project, componentNodes);
  const cellLinks: Map<string, ComponentLinkModel> = generateCellLinks(
    project,
    emptyNodes,
    componentNodes,
  );
  const connectorLinks: Map<string, ExternalLinkModel> = generateConnectorLinks(
    emptyNodes,
    connectorNodes,
  );
  const connectionLinks: Map<string, ExternalLinkModel> =
    generateConnectionLinks(emptyNodes, connectionNodes);
  const externalLinks: Map<string, ExternalLinkModel> = generateExternalLinks(
    emptyNodes,
    externalNodes,
    gateways,
  );
  const externalConnectionLinks: Map<string, ExternalLinkModel> =
    generateExternalConnectionLinks(project, componentNodes, connectionNodes);
...
export function updateBoundNodePositions(
  cellNode: NodeModel<NodeModelGenerics>,
  model: DiagramModel,
) {
  ...
  for (const key in cellNode.getPorts()) {
    if (Object.prototype.hasOwnProperty.call(cellNode.getPorts(), key)) {
      ...
      if (
        connectionId &&
        bound === CellBounds.SouthBound &&
        align === PortModelAlignment.TOP
      ) {
        const portPosition = port.getPosition().clone();
        // change south bound empty node position
        model
          .getNode(getEmptyNodeName(CellBounds.SouthBound, connectionId))
          .setPosition(portPosition);
      }
      // change east bound positions
      if (
        connectionId &&
        bound === CellBounds.EastBound &&
        align === PortModelAlignment.RIGHT
      ) {
        const portPosition = port.getPosition().clone();
      ...
      if (
        connectionId &&
        bound === CellBounds.EastBound &&
        align === PortModelAlignment.LEFT
      ) {
        const portPosition = port.getPosition().clone();
        // change east bound empty node position
        model
          .getNode(getEmptyNodeName(CellBounds.EastBound, connectionId))
          .setPosition(portPosition);
      }
      ...
      if (bound === CellBounds.WestBound && align === PortModelAlignment.LEFT) {
        ...
        model
          .getNode(getEmptyNodeName(CellBounds.WestBound))
      ...
      if (bound === CellBounds.NorthBound && align === PortModelAlignment.TOP) {
        ...
        model
          .getNode(getEmptyNodeName(CellBounds.NorthBound))
          .setPosition(nodePosition);
  ...
  cellPositionNorth.y =
    cellPositionNorth.y - (externalLinkOffset + CIRCLE_WIDTH / 2) - BORDER_GAP;
  model
    .getNode(getEmptyNodeName(CellBounds.NorthBound, BORDER_NODE))
    ?.setPosition(cellPositionNorth.clone());
  // change south bound border empty node position
  const cellPositionSouth = cellNode.getPosition().clone();
  cellPositionSouth.y =
    cellPositionSouth.y + (cellNode.width - CIRCLE_WIDTH) + BORDER_GAP;
  model
    .getNode(getEmptyNodeName(CellBounds.SouthBound, BORDER_NODE))
    ?.setPosition(cellPositionSouth.clone());
}

export function isRenderInsideCell(
  node: BaseModel<BaseModelGenerics>,
): boolean {
  return (
    (node.getType() === COMPONENT_NODE &&
      !(node as ComponentModel).isExternalConsumer()) ||
...
function generateConnectionNodes(
  project: Project,
): Map<string, ConnectionModel> {
  const nodes: Map<string, ConnectionModel> = new Map<
    string,
    ConnectionModel
  >();
  project.components?.forEach((component, _key) => {
...
function generateConnectorNodes(
  project: Project,
): Map<string, ConnectionModel> {
  const nodes: Map<string, ConnectionModel> = new Map<
    string,
...
function generateEmptyNodes(
  projectId: string,
  connectionNodes: ConnectionModel[],
): Map<string, EmptyModel> {
  ...
  connectionNodes.forEach((connectionNode, _key) => {
    if (isConnectorConnection(connectionNode.connection)) {
      ...
      isExternalConnection(projectId, connectionNode.connection, true)
    ) {
      const connectionEmptyNode = new EmptyModel(
...
function getObservationSummary(project: Project): ObservationSummary {
  ...
  const calculateRequestCount = (observations: Observations[] | undefined) => {
...
function generateComponentLinks(
  project: Project,
  nodes: Map<string, CommonModel>,
): Map<string, ComponentLinkModel> {
  ...
  project.components?.forEach((component, _key) => {
    ...
    const callingComponent: ComponentModel | undefined = nodes.get(
      componentId,
    ) as ComponentModel;
    ...
    component.connections.forEach(connection => {
      ...
      if (
        ...
        const associatedComponent = nodes.get(
          getComponentNameById(
            (connectionMetadata as ComponentMetadata).component,
          ),
        ) as ComponentModel;
        if (callingComponent && associatedComponent) {
          const sourcePort: ComponentPortModel | null =
            callingComponent.getPort(`right-${callingComponent.getID()}`);
          const targetPort: ComponentPortModel | null =
            associatedComponent.getPort(`left-${associatedComponent.getID()}`);
          ...
          if (sourcePort && targetPort) {
            ...
            links.set(
              linkId,
              createLinks(sourcePort, targetPort, link) as ComponentLinkModel,
            );
      ...
      if (isConnectorConnection(connection) && connection.onPlatform) {
        ...
        if (callingComponent && associatedComponent) {
          const sourcePort: ComponentPortModel | null =
            callingComponent.getPort(`bottom-${callingComponent.getID()}`);
          const targetPort: ConnectionPortModel | null =
            associatedComponent.getPort(`top-${associatedComponent.getID()}`);
...
function generateConnectorLinks(
  ...
  connectorNodes?.forEach((connectionNode, _key) => {
    ...
    if (sourcePort && targetPort) {
      ...
      links.set(
        linkId,
        createLinks(sourcePort, targetPort, link) as ExternalLinkModel,
      );
...
function generateCellLinks(
  ...
  project.components?.forEach((component, _key) => {
    const componentId = getComponentName(component);
    const targetComponent: ComponentModel | undefined = nodes.get(
      componentId,
    ) as ComponentModel;
    ...
    if (targetComponent) {
      ...
      for (const serviceId in component.services) {
        if (
          Object.prototype.hasOwnProperty.call(component.services, serviceId)
        ) {
          const service = component.services[serviceId];
      ...
      if (isExposed && northBoundEmptyNode) {
        ...
        if (sourcePort && targetPort) {
          ...
          links.set(
            linkId,
            createLinks(sourcePort, targetPort, link) as CellLinkModel,
          );
    ...
    if (targetComponent && targetComponent.isExternalConsumer()) {
      for (const connection of component.connections) {
        ...
        if (targetBound) {
          targetEmptyNode =
            emptyNodes.get(getEmptyNodeName(targetBound)) ?? null;
        }
        if (targetEmptyNode) {
          ...
          if (sourcePort && targetPort) {
            ...
            link.setDestinationNode(destinationComponentNode as ComponentModel);
            links.set(
              linkId,
              createLinks(sourcePort, targetPort, link) as CellLinkModel,
            );
...
function createExternalLink(
  ...
  const sourcePort: CellPortModel | null = emptyNode.getPort(
...
export function isInternalComponent(
  projectId: string,
  connection: Connection,
): boolean {
...
export function isExternalConnection(
  projectId: string,
  connection: Connection,
...
export function isConnectorConnection(connection: Connection): boolean {
  ...
  return (
    connection.type === ConnectionType.Connector ||
    connection.type === ConnectionType.Datastore
  );
}
