...
export function getRoundedOctagonSVG(
  sideLength: number,
  radius: number,
  padding: number = 0,
  ...
  const getQuadrant = (x: number) =>
    Math.floor(((x + 2 * Math.PI) % (2 * Math.PI)) / (Math.PI / 2)) + 1;

  const points: number[][] = [
    [0, radius / sin - radius * sin + padding + offsetY],
  ];
  ...
  for (let i = 1; i <= sideCount; i += 1) {
    ...
    if (prevQ === 1 && nextQ >= 2 && nextQ <= 3) {
      horizontalCut =
        (Math.cos(Math.abs(middle - Math.PI / 2)) * radius) / sin - radius;
    }
...
export function getCellPortId(
  name: string,
  bound: CellBounds,
  align?: PortModelAlignment,
  ...args: string[]
): string {
...
export function getCellPortIdWithoutAlignment(
  name: string,
  bound: CellBounds,
  ...args: string[]
): string {
...
export function getCellPortMetadata(cellPortId: string): {
  name: string;
  bound: CellBounds;
  align?: PortModelAlignment;
  args: string[];
  ...
  let name: string;
  let align: PortModelAlignment | undefined;
...
export function getEmptyNodeName(bound: CellBounds, ...args: string[]): string {
  ...
  return `${EMPTY_NODE}${NAME_JOIN_CHAR}${bound}${
    rest !== '' ? `${rest}` : ''
  }`;
}
