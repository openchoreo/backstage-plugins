...
export class SharedLinkModel extends DefaultLinkModel {
  ...
  fireEventTroughLink = (event: string) => {
    const selectTargetPort = (
      targetNode: NodeModel,
      alignment: PortModelAlignment,
      bound: CellBounds,
    ) => {
      if (
        targetNode.getType() === EMPTY_NODE &&
        (targetNode as EmptyModel).bound === bound
      ) {
  ...
  getCurvePath = (): string => {
    ...
    if (this.getSourcePort() && this.getTargetPort()) {
      ...
      if (
        ...
        this.getTargetPort().getOptions().alignment ===
        PortModelAlignment.BOTTOM
      ) {
      ...
      if (
        this.getSourcePort().getOptions().alignment === PortModelAlignment.LEFT
      ) {
        ...
        this.getSourcePort().getOptions().alignment ===
        PortModelAlignment.BOTTOM
      ) {
  ...
  getArrowHeadPoints = (scale = 1): string => {
    ...
    if (
      ...
      points = `${targetPort.x + 2 * sizeMultiplier} ${targetPort.y}, ${
        targetPort.x + 12 * sizeMultiplier
      ...
      points = `${targetPort.x} ${targetPort.y}, ${
        targetPort.x - 14 * sizeMultiplier
      } ${targetPort.y + 10 * sizeMultiplier},
                ${targetPort.x - 14 * sizeMultiplier} ${
        targetPort.y - 10 * sizeMultiplier
      }`;
    } else if (
      this.getTargetPort().getOptions().alignment === PortModelAlignment.BOTTOM
    ) {
      points = `${targetPort.x} ${targetPort.y + 2 * sizeMultiplier}, ${
        targetPort.x + 10 * sizeMultiplier
  ...
  scaleValueToLinkWidth = (
    ...
    const boundedRequestCount = Math.max(
      minRequestCount,
      Math.min(requestCount, maxRequestCount),
    );
    const range = maxRequestCount - minRequestCount;
    const normalizedCount = (boundedRequestCount - minRequestCount) / range;
    ...
    const scalingFactor =
      maxRequestCount < maxPossibleRequestCount
        ? maxRequestCount / maxPossibleRequestCount
        : 1;
    // Scale to stroke width range
    const dynamicMaxStrokeWidth =
      minStrokeWidth + scalingFactor * (maxFullStrokeWidth - minStrokeWidth);
    const scale =
      (logCount / logMaxCount) * (dynamicMaxStrokeWidth - minStrokeWidth);
