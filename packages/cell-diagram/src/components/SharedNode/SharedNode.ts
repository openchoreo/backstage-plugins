...
export class SharedNodeModel extends NodeModel<NodeModelGenerics> {
  ...
  isNodeSelected = (
    selectedLink: ComponentLinkModel,
    ...
    if (selectedLink) {
      if (selectedLink.getSourcePort().getNode().getID() === this.getID()) {
        ...
        return (
          sourcePortID.slice(sourcePortID.indexOf('-') + 1) === portIdentifier
        );
      } else if (
        selectedLink.getTargetPort().getNode().getID() === this.getID()
      ) {
        const targetPortID: string = selectedLink.getTargetPort().getID();
        return (
          targetPortID.slice(targetPortID.indexOf('-') + 1) === portIdentifier
        );
      }
    }
    return false;
