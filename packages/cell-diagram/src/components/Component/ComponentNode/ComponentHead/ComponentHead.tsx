...
export function ComponentHeadWidget(props: ServiceHeadProps) {
  ...
  return (
    ...
    >
      ...
      <ComponentPortWidget
        port={node.getPort(`left-${node.getID()}`) as ComponentPortModel}
        engine={engine}
      />
      <ComponentPortWidget
        port={node.getPort(`right-${node.getID()}`) as ComponentPortModel}
        engine={engine}
      />
      {node.component.buildPack &&
        node.component.buildPack.toLowerCase() !== 'other' &&
        !previewMode && (
          <ComponentKind>
            {getComponentBuildIcon(node.component.buildPack)}
          </ComponentKind>
        )}
