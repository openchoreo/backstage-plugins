...
export function ComponentWidget(props: ComponentWidgetProps) {
    ...
    const [selectedLink, setSelectedLink] = useState<ComponentLinkModel | undefined>(undefined);
    ...
    useEffect(() => {
        ...
        headPorts.current.push(node.getPortFromID(`left-${node.getID()}`) as PortModel);
        headPorts.current.push(node.getPortFromID(`right-${node.getID()}`) as PortModel);
        headPorts.current.push(node.getPortFromID(`bottom-${node.getID()}`) as PortModel);
        ...
        const externalConsumerListener = node.registerListener({
            EXTERNAL_CONSUMER_LINK_SELECT: (evt: BaseEvent) => {
                handleExternalConsumerLink(evt as ExternalConsumerLinkSelectEvent, 'SELECT');
            },
            EXTERNAL_CONSUMER_LINK_UNSELECT: (evt: BaseEvent) => {
                handleExternalConsumerLink(evt as ExternalConsumerLinkSelectEvent, 'UNSELECT');
            },
        });
    ...
    return (
        ...
        >
            <ComponentHeadWidget
                engine={engine}
                node={node}
                isSelected={node.getID() === selectedNodeId || node.isNodeSelected(selectedLink as ComponentLinkModel, node.getID())}
            ...
            <PortsContainer>
                <ComponentPortWidget port={node.getPort(`top-${node.getID()}`) as ComponentPortModel} engine={engine} />
                <ComponentPortWidget port={node.getPort(`bottom-${node.getID()}`) as ComponentPortModel} engine={engine} />
            </PortsContainer>
        </ComponentNode>
    );
}
